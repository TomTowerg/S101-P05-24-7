...
export default function PackageRegistrationForm({ onSuccess }: { onSuccess?: () => void }) {
  ...
  return (
    <div className="bg-bg-surface rounded-xl shadow-sm border border-border-subtle overflow-hidden transition-theme">
        ...
        <div className="p-6 md:p-10 animate-in fade-in slide-in-from-bottom-6 duration-700">
          <div className="max-w-xl mx-auto space-y-8">
            ...
            <div className="grid grid-cols-1 gap-3">
              <button
                onClick={resetForm}
                className="w-full flex items-center justify-center gap-3 bg-indigo-600 hover:bg-indigo-700 text-white font-black py-4 px-6 rounded-2xl transition-all active:scale-[0.98] shadow-xl shadow-indigo-950/20 cursor-pointer"
              >
      ...
      {/* ── Error Banner ────────────────────────────────────────────────────── */}
      {state.status === "error" && (
        <div className="mx-6 mt-4 flex items-start gap-3 bg-red-500/10 border border-red-500/20 rounded-lg p-4">
          <AlertCircle className="w-5 h-5 text-red-500 shrink-0 mt-0.5" />
          <p className="text-red-400 text-sm flex-1">{state.message}</p>
          <button
            onClick={() => setState({ status: "idle", message: "" })}
      ...
      {state.status !== "success" && (
        ...
        <div className="flex items-start gap-3 p-4 border border-red-500/25 bg-red-500/5 rounded-lg">
          ...
          <div className="flex flex-col">
            <label htmlFor="isPerishable" className="text-sm font-semibold text-red-400 cursor-pointer">
              {t("perishableLabel")}
            </label>
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold py-3 px-6 rounded-lg transition-colors text-sm cursor-pointer"
        >
